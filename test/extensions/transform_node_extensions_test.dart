...
void main() {
  test('returns null for an empty iterable', () {
    expect(<TransformNode>[].nearestTo(.zero), isNull);
  });
  ...
  test('compares elements under different parents in scene space', () {
    final a = TransformNode(position: .new(1, 0));
    final b = TransformNode(position: .new(5, 0));
  ...
  test('accounts for an ancestor\'s angle', () {
    final a = TransformNode(position: .new(10, 0));
    final b = TransformNode(position: .new(0, 11));
  ...
  test('nearest measures from the given node\'s absolute position', () {
    final target = TransformNode(position: .zero);
    final a = TransformNode(position: .new(1, 0));
    final b = TransformNode(position: .new(5, 0));
