...
void main() {
  test('emits onStart once it starts progressing', () {
    final effect = ControlledEffect(controller: .sequence([.delay(0.5), .linear(1)]));
    effect.mount();
    var starts = 0;
  ...
  test('onStart is not re-emitted per repeat lap; only once for the whole run', () {
    final effect = ControlledEffect(controller: .repeat(.linear(1), 2));
    effect.mount();
    var starts = 0;
    effect.onStart(() => starts += 1);
  ...
  test('reversing within a repeat lap never redoes the initial delay', () {
    final effect = ControlledEffect(controller: .sequence([.delay(0.5), .repeat(.linear(1), 2)]));
    effect.mount();
  ...
  test('repeats forever when times is null', () {
    final effect = ControlledEffect(controller: .infinite(.linear(1)));
    effect.mount();
    var finishes = 0;
    effect.onFinish(() => finishes += 1);
  ...
  test('detaches itself once complete when added as a child, when cleanup is true', () {
    final a = Node();
    final effect = ControlledEffect(controller: .linear(2), cleanup: true);
    a.add(effect);
    final scene = a.mount();
